...
def load_trained_pyramid(opt, mode_="train"):
    ...
    if os.path.exists(dir):
        Gs = torch.load("%s/Gs.pth" % dir, map_location=opt.device, weights_only=False)
        Zs = torch.load("%s/Zs.pth" % dir, map_location=opt.device, weights_only=False)
        reals = torch.load("%s/reals.pth" % dir, map_location=opt.device, weights_only=False)
        NoiseAmp = torch.load("%s/NoiseAmp.pth" % dir, map_location=opt.device, weights_only=False)
    else:
